...
> Additive, non-destructive. One new read-only VIEW; no data moves; no backup needed.
...
- [ ] T013 Write `migrations/NNN_artifacts_view.sql`: one `CREATE VIEW artifacts` UNION-ing `raw_data` (identity map, `source='raw_data'`) with `performance_datasets` mapped into `raw_data`'s shape (`data_type='performance'`, `source='performance'`, `source_row_id=pd.id`, `total_bytes→total_size`, `storage_dir→storage_path`, `collected_at→measured_at`, and `aliquot_code`/`test_type`/`data_format`/`source_folder_name` folded into a code-authored `json_object(...)` expression). Exact SQL in [`./data-model.md`](./data-model.md). **No `artifact_files` view** (detail/files stay per-source). No `BEGIN`/`COMMIT`. *(FR-005, FR-006, FR-010, Article V, Article VII, depends on T012)*
- [ ] T014 Apply migrations locally; confirm the `artifacts` view is created, `schema_migrations` records the file with a checksum, and **no** `raw_data`/`performance_datasets`/file row was altered (nothing is copied; no pre-migration snapshot required). *(FR-006, FR-011, NFR-001, NFR-002, AC-008, Article IV, depends on T013)*
- [ ] T015 Assert each `performance_datasets` row is queryable via the `artifacts` list view with `data_type='performance'` and carries `source='performance'` + `source_row_id`. (Its files stay served by the existing performance detail endpoint.) *(FR-005, AC-007, depends on T014)*
- [ ] T016 Repoint the Artifact **list** query (`app/features/raw_data.py :: get_raw_data_list`) to `FROM artifacts` (returns both sources, each tagged with `source`); leave `get_raw_data_detail` / files / download / `delete_raw_data` **unchanged**. Reframe the performance create path (`app/features/performance.py`) so new uploads write `raw_data` (`data_type='performance'`). Keep the existing performance endpoints live over the retained tables per [`./contracts/routes-and-api.md`](./contracts/routes-and-api.md). *(FR-005, FR-008, depends on T015)*
...
- [ ] T018 [P] Frontend: the Artifacts list renders both sources and routes each row's detail/actions to the existing per-source page by `source` (raw-data-origin → RawData detail; performance-origin → Performance detail). Present performance as an artifact type under Raw Data / Artifacts (`frontend/src/pages/*`, `frontend/src/utils/constants.ts` nav). *(FR-005, FR-008, depends on T015)*
