...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def route(self, method):
        ...
        try:
            authorize(method, path, self.headers)
    ...
    def handle_api(self, method, path, query):
        ...
        if method == "POST" and path == "/api/parsed-data/mock":
            ...
            # client JSON. Disabled in production; enable with LIMS_ENABLE_MOCK.
            if os.environ.get("LIMS_ENABLE_MOCK", "").strip().lower() not in {"1", "true", "yes", "on"}:
