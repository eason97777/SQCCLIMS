...
Environment variables (read at call time, never bound at import):
    LIMS_AUTH_ENABLED   truthy (1/true/yes) to turn enforcement ON
    LIMS_AUTH_DISABLED  truthy hard-override that keeps auth OFF even if enabled
    LIMS_API_TOKENS     "token1:role1,token2:role2" -> {token: role}
...
def auth_enabled():
    ...
    """
    if _is_truthy(os.environ.get("LIMS_AUTH_DISABLED")):
        return False
    return _is_truthy(os.environ.get("LIMS_AUTH_ENABLED"))


def token_store():
    """Parse LIMS_API_TOKENS ("tok:role,tok:role") into {token: role}.

    Malformed entries (no colon, blank token, unknown role) are skipped.
    """
    store = {}
    raw = os.environ.get("LIMS_API_TOKENS", "")
